...
bool TextParser::create(const char *name)
{
  ...
  if (f == NULL)
    {
      char s[256];
      std::cerr << "The resource file \"" << name << "\" does not exist\n" << std::endl;
      return false;
    }
  size = stbuffer.st_size;
  data = new char[size];
...
char* TextParser::getword( bool to_uppercase)
{
	int p0,p1,i;
	...
	for (i=p0;i<p1;i++)
	{
		if (to_uppercase && (data[i]<='z') && (data[i]>='a'))
			data[i] += ('A'-'a');
...
bool TextParser::seek(const char *token)
{
  char *dummy=getword();
  if (!dummy)
	  return false;

  while (dummy && strcmp(dummy, token) && (sl < size))
	  dummy = getword();
